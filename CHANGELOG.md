# Changelog

All notable changes to `memran/marwa-mcp` will be documented in this file.

## 0.1.0 - Unreleased

- Added JSON-RPC 2.0 MCP server core.
- Added stdio and HTTP POST transports.
- Added tool, resource, prompt, and permission abstractions.
- Added built-in `ping`, `server_info`, and `echo` tools.
- Added built-in Marwa server info and tools resources.
- Added built-in Marwa debug and module generator prompts.
- Added PHPUnit, PHPStan, PHPCS, and GitHub Actions CI configuration.
